...
class Content extends AdminController
{
	...
	function CheckSortableAction()
	{
		...
		if ($data->isAjaxCall)
		{
			...
			else if ($data->refresh)
			{
				...
				$trans = $this->trans;
				include($this->GetBlockPath('content_block'));
	...
	function GetViewData($query = '')
	{
		...
		array_push(
			...
			//_JS_APPLICATION_FOLDER.'categories'.'/categories.js'
		);
        ...
        //echo'<pre>';print_r($data->contentListContent);echo'</pre>';die;
		
		// $dataSort = $this->GetSortData();

		// $limit = $this->GetPagingCode($data, $recordsCount);
        $data->contentListContent = HtmlControls::GenerateDropDownList($data->contentList, 'id', 'name', $data->categoryId);
	...
	function GetEditData($editId = 0)
	{
		...
		array_push(
			...
			'lib/tinymce/tinymce.min.js',
			'lib/wrappers/tinymce/tinymce.js',
			_JS_APPLICATION_FOLDER.$this->module.'/category_edit.js');
